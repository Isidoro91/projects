...
class StackOverflow extends Serializable {
  ...
  def scoredPostings(grouped: RDD[(Int, Iterable[(Posting, Posting)])]): RDD[(Posting, Int)] = {
    ...
    grouped.flatMap(_._2)
      .groupByKey()
      .mapValues(answerList => answerHighScore(answerList.toArray))
  ...
  def vectorPostings(scored: RDD[(Posting, Int)]): RDD[(Int, Int)] = {
    ...
    scored.map(score => (firstLangInTag(score._1.tags, langs).getOrElse(0) * langSpread, score._2)).persist()
      ...
      //.filter(score => firstLangInTag(score._1.tags, langs).isDefined)
      //.map(score => (langSpread * firstLangInTag(score._1.tags, langs).get, score._2))
      //.persist()
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    def medianFunction(a: Iterable[(Int, Int)]) = {
    ...
    val median = closestGrouped.mapValues { vs =>
      val langId: Int = vs.groupBy(_._1).maxBy(_._2.size)._1//vs.map(_._1).groupBy(identity).maxBy(_._2.size)._1
      val langLabel: String   = langs(langId / langSpread) // most common language in the cluster
      val clusterSize: Int    = vs.size
      val langPercent: Double = vs.count(v => v._1 == langId) * 100d / clusterSize.toDouble // percent of the questions in the most common language
      val medianScore: Int    = medianFunction(vs)
